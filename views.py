...
def picking(data):
    ...
    for item in list(set(verificar_se_ativo)):
        try:
            if item.numero_da_nota and item.produto in lista_conferencia:
                pass
            else:
...
def verify_if_billing_done(id_pickinglist):
    ...
    status_separacao = session.query(Separacao).filter(Separacao.id_mercado == id_pickinglist).first().status
    if status_picking == True and status_separacao == True:
        faturamento = session.query(Faturamento).filter(Faturamento.id == id_pickinglist,Faturamento.data == str(date.today())).first()
...
def separate(product,data,note_number,user,qtd_coletada):
    try:
        ...
        if verificar_separacao.quantidade == verificar_separacao.qtd_coletada:
            ...
            session.commit()
            verify_if_billing_done(session.query(Picklist).filter(Picklist.nota == verificar_separacao.nota,Picklist.data == str(date.today()),Picklist.id == verificar_separacao.id_mercado).first().id_faturamento)
        ...
        if item_separado.quantidade == item_separado.qtd_coletada:
            ...
            verify_if_billing_done(session.query(Picklist).filter(Picklist.nota == note_number,Picklist.data == str(date.today())).first().id_faturamento)
